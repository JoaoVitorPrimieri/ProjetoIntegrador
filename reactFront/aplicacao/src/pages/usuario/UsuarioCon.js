...
function UsuarioCont() {
  ...
  const initialState = { usuId: null, usuNome: '', usuEmail: '', usuCpf: '', usuTelefone: '', usuEndereco: '', usuSexo: '', usuSenha: '' }
  ...
  const salvar = () => {
    if (usuario.usuId == null) { // inclussão
  ...
  const excluirConfirm = (usuId) => {
    ...
    UsuarioSrv.excluir(usuId).then(response => {
      onClickAtualizar();
      toastRef.current.show({
        severity: 'success',
